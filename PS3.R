...
## Convetional
summary(lm(Y~D-1, data = simulation))$coefficients[,2] ## 0.2755

## Robust
summary(lm_robust(Y~D-1, data = simulation))$coefficients[,2] ##0.2548
...
(iii) Like $\widehat{\beta}$, each of the five standard errors from above has *its own* sampling distribution. What are the means and standard deviations of these distributions?
  ...
  (i) -0.026 which is completely coherent with the true value of $\beta = 0$. I have used 500 iterations to reach this value (see code below).
