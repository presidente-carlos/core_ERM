...
#Checking type and class of the variables. 
# (Note that this information is also contained within the str() display)
typeof(bm$sex)
typeof(bm$race)
class(bm$sex)
class(bm$race)
...
```{r}
bm %>% select(female, race) %>% group_by(race) %>%
  summarize(share_fem=mean(female)*100)
```
...
```{r}
 bm %>% select(computerskills, race) %>% group_by(race) %>% 
  summarize(share=mean(computerskills))
 
#Alternative piece of code using the sum command
 sum_CS<-bm %>% select(computerskills, race) %>% 
   group_by(race) %>% 
   summarize(n = n(), sum=sum(computerskills), share = sum/n)
 sum_CS
...
```{r}
#First note that this piece of code produces an unsatisfactory solution
   bm %>% select(education, race) %>% group_by(race) %>%
  summarize(share=mean(education))
#Because, education is in fact a categorical variable,
# we might want to know the share of people in each of 
# the five categories across each race. To do so: 
str(bm$education)

##We first identify the number of people in each education level by race
sum_educ<-bm %>% select(education, race) %>% 
  group_by(race) %>% count(education)
...
##Means are however useful given that ofjobs is a
# truly numeric variable
 bm %>% select(ofjobs, race) %>% group_by(race) %>%
   summarize(share=mean(ofjobs))
```
...
```{r}
bm %>% select(yearsexp, race) %>% group_by(race) %>%
  summarize(avg_yearsexp = mean(yearsexp), sd_yearsexp = sd(yearsexp))
```
...
```{r}
bm %>% select(sex, computerskills, education) %>%
  group_by(sex) %>% 
  summarize(avg_CS = mean(computerskills), avg_educ = mean(education))
```
...
## I don't understand the reason for using the function pull here. 
# See a more "inefficient" way to run the same code using pull
...
```{r}
library(tidyverse)
two_truths<-read.csv("https://ditraglia.com/data/two-truths-and-a-lie-2022.csv",
                     stringsAsFactors = F)
head(two_truths)
```
...
two_truths<- two_truths %>% select(-Timestamp) %>%
  rename("certainty" = Certainty.in.your.guess, "guessed_right" = Outcome)
head(two_truths)
...
```{r}
two_truths %>% group_by(certainty) %>% 
  summarize(avg_guess = mean(guessed_right)) %>%
  ggplot() +
...
# Apologies for the long code, hopefully it is legible enough
# (I tried to over use the dplyr syntax)
# Note that in practice it makes little sense
# to have CI for proportions out of the [0,1] interval

certainty_levels <- two_truths %>% group_by(certainty) %>%
  count() %>% filter(n>3) %>% pull(certainty)
two_truths %>% group_by(certainty) %>%
  filter(certainty %in% certainty_levels) %>%
  summarize(n = n(), proportion = mean(guessed_right), 
            se = sqrt(proportion*(1-proportion)/n), 
            CI_low = proportion-1.96*se,
            CI_high = proportion + 1.96*se) %>% 
  ggplot() + 
  geom_point(aes(x=certainty, y=proportion)) +
  geom_errorbar(aes(x=certainty, y=proportion,  
                    ymin=CI_low, ymax=CI_high), width=.2,
